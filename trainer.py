...
from BUS_utils import DiceLoss, dice_coef, AutomaticWeightedLoss, \
...
def train_epoch(model,
    ...
    for idx, batch in enumerate(loader):
        ...
        image = batch["image"]
        target = batch["label"]
        cls_labels = batch["level"]
        adc = batch["adc"]
        for param in model.parameters(): param.grad = None

        cls_logits = model(image, adc)[0]
        seg_logits = model(image, adc)[1]
        seg_loss = loss_func(seg_logits, target)
        cla_loss = cls_loss_func(cls_logits, cls_labels)
        awl = AutomaticWeightedLoss().cuda() # 参考BUS
        loss = awl(seg_loss, cla_loss)
